<style>
    ...
    tbody tr:hover .radius1 {
    border-radius: 5px 0px 0px 5px;

}

    tbody tr:hover .radius2 {
...
$viewsPermitidas = [
    ...
    'login'  => __DIR__ . '/../login/login.php',
];

$nomeUsuarioLogado = $_SESSION['usuario_nome'] ?? 'Minha conta';
?>
        ...
        <div class="sidebar">
            ...
            <div id="principal">
                ...
                <div class="baixo" id="menuConta">
                    ...
                    <form action="?page=logout" method="POST" class="conta-menu" id="contaMenu" style="display: none;">

                        <button  class="conta-menu-item conta-menu-sair" id="btnSair">
                            <i class="fa-solid fa-right-from-bracket" ></i>
                            <a style="text-decoration: none;" href="?page=login">Sair</a>
                        </button>
                    </form>
                </div>
            </div>
            ...
            if ($view === null) {
                ...
                if ($resVei && $resVei->num_rows > 0) {
                    while ($row = $resVei->fetch_object()) {
                        echo '<tr>
                                <td class="bold mono  radius1">' . htmlspecialchars($row->placa) . '</td>
                                <td><span class="badge bg-secondary">' . htmlspecialchars($row->modelos) . '</span></td>
                                <td>' . htmlspecialchars($row->cor) . '</td>
                                <td class="bold radius2">' . htmlspecialchars($row->ano) . '</td>
                            </tr>';
                    }
                } else {
                ...
                if ($resUser && $resUser->num_rows > 0) {
                    while ($row = $resUser->fetch_object()) {
                        ...
                        echo '<tr>
                                <td class="radius1">' . htmlspecialchars($row->nome) . '</td>
                                <td>' . htmlspecialchars($row->email) . '</td>
                                <td class="mono">' . htmlspecialchars($row->login) . '</td>
                                <td class="radius2">' . $statusBadge . '</td>
                            </tr>';
                    }
                } else {
